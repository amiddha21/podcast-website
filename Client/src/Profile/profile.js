...
const Profile = () => {
  ...
  useEffect(() => {
    const user_id = context.dbProfileState && context.dbProfileState[0].uid;
    if (!user_id) {
      return;
    }
    axios
  ...
  const uid = context.dbProfileState && context.dbProfileState[0].uid;

  return (
    <div>
      <div>
        <Link to={{ pathname: '/showmessages/' + uid }}>
